...
### Variant Prompts (SDK Feature)

When the user wants to explore alternatives, generate **2-3 variant prompts** alongside the main prompt. Each variant changes one major design decision to produce a meaningfully different alternative:

| Variant Dimension | What Changes | Example |
|-------------------|-------------|---------|
| **Color scheme** | Swap accent and background colors | Main: dark teal on charcoal → Variant: warm gold on navy |
| **Layout structure** | Rearrange spatial relationships | Main: sidebar + cards → Variant: full-width stacked sections |
| **Typography mood** | Change the font pairing | Main: Clash Display + DM Sans → Variant: Space Grotesk + JetBrains Mono |
| **Density** | Adjust information density | Main: spacious with 48px gaps → Variant: compact with 16px gaps |
...
const screen = await project.generate(mainPrompt);
const variants = await screen.variants("Explore different visual directions", {
  variantCount: 3,
  creativeRange: "EXPLORE",        // REFINE (subtle) | EXPLORE (moderate) | REIMAGINE (dramatic)
  aspects: ["COLOR_SCHEME", "LAYOUT"],  // What to vary
});
// Compare variants[0], variants[1], variants[2] screenshots
```

**When to suggest variants:**
- User is unsure about aesthetic direction
- First screen of a new project (sets the DNA)
- Hero sections or landing pages where visual impact matters most
