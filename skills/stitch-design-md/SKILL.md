...
To analyze a Stitch project, retrieve screen metadata and design assets using the SDK (preferred) or MCP tools:
...
// 3. Get assets
const htmlUrl = await screen.getHtml();    // HTML download URL
const imageUrl = await screen.getImage();  // Screenshot URL

// 4. Download assets via Bash
// curl -sL "$htmlUrl" -o queue/source.html
// curl -sL "$imageUrl" -o queue/source.png
```
