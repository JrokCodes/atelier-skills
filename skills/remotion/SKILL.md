---
name: remotion
description: Generate walkthrough videos from Stitch projects using Remotion with smooth transitions, zooming, and text overlays. Use when generating walkthrough video, demo video, Remotion, video from Stitch, screen recording, client demo video.
allowed-tools:
  - "stitch*:*"
  - "remotion*:*"
  - "Bash"
...
**Required:**
- Access to the Stitch SDK or MCP Server
...
// For each screen, get assets
for (const screen of screens) {
  const detail = await project.getScreen(screen.id);
  const imageUrl = await detail.getImage();    // Screenshot URL
  const htmlUrl = await detail.getHtml();       // HTML download URL
}
```

Then download via Bash:
```bash
bash scripts/download-stitch-asset.sh "$imageUrl" "assets/screens/screen-name.png"
```
